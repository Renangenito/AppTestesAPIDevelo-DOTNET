...
export default function AnalisesEnviar (){
  ...
  const enviar = async (data) => {
    ...
    if(data.ImagemQrCode.length !== 0){
      ...
      data.ImagemQrCode = data.ImagemQrCode.substring(data.ImagemQrCode.indexOf(',')+1)
    ...
    api.post('Analises/Enviar/', params)
    .then(resposta =>{
      setSucesso(resposta);
      setErro("");
      setLoading(false);
    ...
    return(        
      <>
      <h1 className='text-center mb-4'> Teste Analises Enviar </h1>
      ...
      <form className="form" noValidate autoComplete="off" onSubmit={handleSubmit(enviar)}>
        ...
        <div class="col-sm">
          ...
          <input type="text" className="form-control mb-1" placeholder="Chave" aria-label="" aria-describedby="basic-addon1"
          {...register("Chave", {
        ...
        <div class="col-sm">
          ...
          <input type="text" className="form-control mb-1" placeholder="CPF" aria-label="" aria-describedby="basic-addon1"
          {...register("CPF", {
        ...
        <div className="d-inline">
          <button className="btn btn-outline-success mb-1" type="submit">Enviar</button>
        </div>
        <div className="d-inline">
          <Link to={"/Analises"}><button className="btn btn-primary mb-1" type="button">Voltar</button></Link>   
        </div>
        </div>
      </form>
